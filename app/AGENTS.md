...
| Tailwind CSS | 4 |
| UI Primitives | Radix UI (shadcn/ui new-york 스타일, `components.json`) |
...
│   ├── chat/          # 챗봇 페이지
│   ├── bookmarks/     # 북마크 관리 (+ deleted/ 휴지통)
│   ├── signin/signup/verify-email/reset-password/(+confirm)/
│   ├── oauth/callback/ # Google OAuth 콜백 페이지
│   └── globals.css
