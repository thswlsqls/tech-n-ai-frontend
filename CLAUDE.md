...
docs/     # 한국어 PRD, API 명세, 버그 기록, LLM 프롬프트 (docs/README.md 참고)
devops/   # AWS 배포 설계 프롬프트(prompts/)와 산출물(results/), Terraform IaC(terraform/)
scripts/  # tmux 개발 환경 스크립트와 가이드 문서
.claude/  # 에이전트 스킬, 로컬 설정
.agents/  # 에이전트 스킬
```

두 앱은 같은 스택을 공유한다: React 19, App Router, Tailwind v4 (`@tailwindcss/postcss`, `src/app/globals.css`의 CSS 변수 테마), shadcn/ui 방식 컴포넌트 (`src/components/ui` — new-york 스타일 `components.json`은 `app`에만 있다), Radix UI, lucide 아이콘. 경로 별칭 `@/*` → `./src/*`. `admin`은 여기에 더해 `recharts`(대시보드)와 `react-markdown`/`remark-gfm`을 쓴다.
...
npm run lint     # eslint — flat config(eslint.config.mjs)는 app에만 있다
```
...
- **docs/**: 새 문서는 순번이 붙은 `NNN-name.md`로 만든다 (번호/형식 규칙은 `docs/README.md` 참고).
