...
| 차트 | Recharts (Agent 실행 결과 pie/bar) |
...
├── app/                # 페이지 (App Router)
│   ├── page.tsx       # 대시보드 (인증 필요)
│   ├── api/bff/       # BFF 인증 엔드포인트 (login, logout, refresh, me)
│   ├── agent/         # 에이전트 실행 페이지
│   ├── accounts/      # 계정 관리
│   ├── signin/        # 로그인 페이지
│   └── test-chart/    # 차트 렌더링 확인용 개발 페이지
├── components/
│   ├── ui/            # 기본 UI (alert-dialog, badge, button, dialog, input, toast)
│   ├── auth/          # 인증 컴포넌트 (header, signin-form)
│   ├── admin/         # 계정 관리 컴포넌트 (테이블, CRUD 다이얼로그)
│   └── agent/         # 에이전트 기능 컴포넌트 (사이드바, 메시지, 차트)
├── contexts/          # React Context (auth, toast)
├── lib/               # API 클라이언트, 유틸리티, auth-fetch, cookie-config
├── types/             # TypeScript 타입 정의 (auth, admin, agent, common)
└── middleware.ts      # /api/v1/* 요청에 쿠키의 토큰을 Authorization 헤더로 주입
```
